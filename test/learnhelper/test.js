...
const assertDocs = (docs) => {
  ...
  doc.state.should.match(/已经提交|尚未提交/)
}
...
const assertAssignments = (assignments) => {
  ...
  const properties = ['title', 'startDate', 'dueDate', 'state', 'size', 'assignmentID', 'detail', 'filename', 'fileURL', 'evaluatingTeacher',
    'evaluatingDate', 'scored', 'grade', 'comment']
  ...
  assignment.state.should.match(/previous|new|unknown/)
}
  ...
  describe('3. test method "getAssignments"', function () {
    ...
    it('3.1 assignment info should be returned', async () => {
      ...
      assignmentIDs.forEach(async (ele, index) => {
        ...
        // console.log('responseDetail = ', responseDetail.toString())
        
        nock('https://learn.tsinghua.edu.cn')
          .get('/MultiLanguage/lesson/student/hom_wk_detail.jsp')
          .query((query) => {
            console.log('query = ', query)
            matchResult = 'course_id' in query && query.course_id === courseID && 'id' in query && query.id === ele && 'rec_id' in query && query.rec_id === recIDs[index]
            console.log('matchResult = ', matchResult)
            return ('course_id' in query && query.course_id === courseID && 'id' in query && query.id === ele && 'rec_id' in query && query.rec_id === recIDs[index])
          })
